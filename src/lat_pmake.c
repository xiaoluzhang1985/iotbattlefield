...
lat_pmake(int ac, char **av)
{
	...
	while (++optind < ac) {
		...
		printf("usecs: %lld, get_n: %lld, gettime: %lld \n",usecs, get_n(), gettime());
		state.iterations = (iter_t)((usecs * get_n()) / gettime());
		
		benchmp(setup, bench, NULL, 0, parallel, warmup, repetitions, &state);
	
		printf("after bench -> usecs: %lld, get_n: %lld, gettime: %lld \n",usecs, get_n(), gettime());
		
		time = gettime();
		time /= get_n();
		if (time > 0.0)
		fprintf(stderr, "usecs:%llu, time:%d\n", usecs, time);
	}
	return (0);
}
