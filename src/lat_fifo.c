...
int 
lat_fifo(int ac, char **av, char* retd)
{
	state_t state;
	int parallel = 1;
	int warmup = 0;
...
initialize(iter_t iterations, void *cookie)
{
	...
	if (write(state->wr, &c, 1) != 1 || read(state->rd, &c, 1) != 1) {
		perror("(i) read/write on pipe init");
		exit(1);
	}
}
...
doit(register iter_t iterations, void *cookie)
{
	...
	while (iterations-- > 0) {
		if (write(w, cptr, 1) != 1 ||
		    read(r, cptr, 1) != 1) {
			perror("(r) read/write on pipe doit");
			exit(1);
		}
	}
}
...
writer(register int w, register int r)
{
	...
	for ( ;; ) {
		if (read(r, cptr, 1) != 1 ||
			write(w, cptr, 1) != 1) {
			    perror("(w) read/write on pipe writer");
			    exit(1);
		}
	}
}
