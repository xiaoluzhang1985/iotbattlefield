...
int
lat_ops(int ac, char **av, char* retd)
{
	int	__n = 1;
	int	c, i, j;
	int	warmup = 0;
	...
	nano("integer bit", get_n() * 100 * 3, retd);
	
	benchmp(NULL, do_integer_add, NULL, 
		0, 1, warmup, repetitions, &state);
	nano("integer add", get_n() * 100 * 2 ,retd);
	iop_time = gettime();
	iop_N = get_n() * 100 * 2;
