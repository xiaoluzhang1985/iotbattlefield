...
static double	loads(benchmp_f initialize, int len, int warmup, int repetitions, void* cookie);
...
int
lat_dram_page(int ac, char **av, char* retd)
{
	int	i, j, l;
	int	verbose = 0;
	...
	if (dram_hit < 0.95 * dram_miss) {
		sprintf(outstr, "DRAM page latency: %f\n", dram_miss - dram_hit);
	} else {
		sprintf(outstr, "DRAM page latency: 0.0\n");
	}
	strncat(retd, outstr, strlen(outstr));
	return (0);
}
