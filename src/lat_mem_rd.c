...
#define	LOWER	512
double	loads(size_t len, size_t range, size_t stride, 
...
int
lat_mem_rd(int ac, char **av, char* retd)
{
	int	i;
	int	c;
	int	parallel = 1;
	...
	if (optind == ac - 1) {
		...
		for (range = LOWER; range <= len; range = step(range)) {
			...
			sprintf(stderr, "Memory read latency %.5fMB %.3fns\n", range / (1024. * 1024.), result);
			strncat(retd,outputstr,strlen(outputstr));
		}
	} else {
		for (i = optind + 1; i < ac; ++i) {
			...
			for (range = LOWER; range <= len; range = step(range)) {
				result=loads(len, range, stride, parallel,warmup, repetitions);
				sprintf(outputstr, "Memory read latency: %.5fMB %.3fns \n ", range / (1024. * 1024.), result);
				strncat(retd, outputstr, strlen(outputstr));
			}
		}
	}
