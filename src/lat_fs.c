...
void	measure(size_t size, 
		int parallel, int warmup, int repetitions, void* cookie, char* ret);
...
int
lat_fs(int ac, char **av, char* ret)
{
	int i;
	int parallel = 1;
	int warmup = 0;
	...
	if (state.size) {
		measure(state.size, parallel, warmup, repetitions, &state, ret);
	} else {
		for (i = 0; i < sizeof(sizes)/sizeof(int); ++i) {
			state.size = sizes[i];
			measure(state.size, 
				parallel, warmup, repetitions, &state, ret);
		}
	}
	return(0);
}
...
void
measure(size_t size, int parallel, int warmup, int repetitions, void* cookie, char *ret)
{
	...
	if (gettime()) {
		nano("File make latency",get_n(),ret);
		//fprintf(stderr, "File make latency: \t%lu\t%.0fns", (unsigned long)get_n(),
		//	(double)( 1000. * (double)gettime() / get_n()));//iteration for 1 sec runtime
	...
	benchmp(setup_rm, benchmark_rm, cleanup_names, 0, parallel,
		warmup, repetitions, cookie);
	if (gettime()) {
		nano("File remove latency", get_n(),ret);
		//fprintf(stderr, "File remove latency\t%.0f", 
		//	(double)(1000. * (double)gettime() / get_n()));
	} else {
		fprintf(stderr, "\t-1");
	}
	//fprintf(stderr, "\n");
