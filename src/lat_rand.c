...
#endif
int 
lat_rand(int ac, char **av, char* retd)
{
	int parallel = 1;
	int warmup = 0;
...
//#ifdef HAVE_DRAND48
	...
	nano("drand48 latency", get_n(), retd);

	benchmp(NULL, bench_lrand48, NULL,
		0, parallel, warmup, repetitions, NULL);
	nano("lrand48 latency", get_n(), retd);

//#endif
#ifdef HAVE_RAND
	benchmp(NULL, bench_rand, NULL,
		0, parallel, warmup, repetitions, NULL);
	nano("rand latency", get_n(), retd);
#endif
#ifdef HAVE_RANDOM
	benchmp(NULL, bench_random, NULL,
		0, parallel, warmup, repetitions, NULL);
	nano("random latency", get_n(), retd);
#endif
	return (0);
}
